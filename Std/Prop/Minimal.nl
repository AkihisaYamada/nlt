...
---
import Imp, And, Or, Not, MinimalNot, Iff.
...
lemma not_iff_imp_not_true: if [P : Prop] then ¬P ⟷ (P ⟶ ¬true);
	...
	- if P0; apply imp_not_imp_not;
		- if P;
			apply P0[THEN imp_elim1, OF P, THEN not_elim_not].
		.
	.
...
extend Quantifiable begin
	...
	lemma ex_imp_iff_all_imp#simp
		...
		- if ex_imp; apply all_intro;
			- if [x : A]; apply imp_intro;
				- if Px; apply ex_imp[THEN imp_elim1]; by ex_intro1[OF Px].
				.
			.
		- if all_imp; apply imp_intro;
			- if ex; apply ex_elim[OF ex];
				- if Px: P.[x], ...;
					apply all_imp[THEN all_elim1[of x], THEN imp_elim1];
					by Px.
				.
			.
		.
