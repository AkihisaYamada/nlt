...
public:
		...
		if( _parser.skips("if") ) {
			...
			for(;;) {
				if( _parser.skips("[") ) {
					...
					cout << cs << _syntax->pretty_term(assm) << ", " << flush;
				}
				if( !_parser.skips(",") ) break;
			};
		}
	...
	void _obtain() {
		...
		auto const& thm = Prover(*this,_thy,Inference::claim_exact(_thy,goal)).deepen()._prompt().proof_loop();
