...
theory Eq:
	...
	assume eq_cong_meta: for P A if A ∈ EQTYPE, x = y, P.[x], x ∈ A, y ∈ A, (∀z. z ∈ A ⟹ P.[z] ∈ PROP) then P.[y].
begin
	...
	lemma eq_trans:
		...
		apply eq_cong_meta[of (w. x = w) A, OF A yz xy];
		by eq_prop[OF A].

	theory Pair:
		fix (×) (,) fst snd.
		assume prod_type: if A ∈ EQTYPE, B ∈ EQTYPE then A × B ∈ EQTYPE.
		assume fst: if A ∈ EQTYPE, B ∈ EQTYPE, a ∈ A, b ∈ B then fst (a,b) = a.
		assume snd: if A ∈ EQTYPE, B ∈ EQTYPE, a ∈ A, b ∈ B then snd (a,b) = b.
		assume pair_cong: if A ∈ EQTYPE, B ∈ EQTYPE, a ∈ A, b ∈ B, a' ∈ A, b' ∈ B, a = a', b = b' then (a,b) = (a',b').
	begin
