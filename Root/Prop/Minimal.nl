...
namespace iff begin
	...
	lemma elim:
		if PQ: P ⟺ Q then for thesis
		if assm: (P ⟹ Q) ⟹ (Q ⟹ P) ⟹ thesis, ! P ∈ Prop, ! Q ∈ Prop then thesis;
	apply assm;
		- by iff_elim1[OF PQ].
		- by iff_elim2[OF PQ].
	.
	...
	lemma imp_cong: for P Q
		if [P ∈ Prop, P' ∈ Prop], PP': P ⟺ P', QQ': P' ⟹ Q ⟺ Q', [Q ∈ Prop, Q' ∈ Prop]
		then (P ⟹ Q) ⟺ (P' ⟹ Q');
	apply iff_intro;
...
lemma iff_cong_not(cong) if !P ∈ Prop, PP': P ⟺ P', !P' ∈ Prop then ¬P ⟺ ¬P';
	apply iff_intro;
	if nP: ¬P;
		apply not_intro;
